...
```{r}
wp_nga <- read_rds("data/wp_sf.rds") 
```
...
We will first select the variables only required for our anlaysis. As we require the following for our analysis
...
We will aslo tidy the name of variables that starts with "\#"

```{r}
wp_nga_cleaned <- wp_nga %>%
  select(5,6,13,19:23) %>%
...
```{r}
freq(data=wp_nga_cleaned, 
     input = 'status_clean')
```

#### 1.3.1.1 Recoding NA values into String

We observed that there are more than 10% of NAs for this field, in which we will recode it into 'Unknown'.

```{r}
wp_nga_cleaned <- wp_nga_cleaned %>%
...
```{r}
wpt_nonfunctional <- wp_nga_cleaned %>%
  filter(status_clean %in%
           c("Non-Functional",
             "Non-Functional due to dry season",
             "Abandoned/Decommissioned",
...
To count the number of different categories of water points found in each LGA, we will utilize the `mutate()` function for the calculation as shown in the code:

```{r}
nga_wp <- nga %>% 
  mutate(`total wpt` = lengths(
    st_intersects(nga, wp_nga_cleaned))) %>%
