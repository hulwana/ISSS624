...
```{r}
pacman::p_load(sf, spdep, tmap, tidyverse, readr, dplyr, purrr)
```
...
#### 1.11.2.1 Deriving the centroid
...
#### 1.11.2.2 Determine the cut-off distance
...
#### 1.11.2.3 Computing fixed distance weight matrix
...
### 1.11.3 Computing adaptive distance weight matrix
...
## 1.12 Computing Gi Statisitcs

### 1.12.1 Gi Statistics using fixed distance
...
### 1.12.2 Mapping GI values with fixed distance weights
...
### 1.12.3 Gi Statistics using adaptive distance
...
### 1.12.4 Mapping Gi values with adaptive distance weights
